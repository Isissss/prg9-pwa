...
const cacheProjectImages = async () => {
  ...
  try {
    ...
    const images =
      projects?.map(({ project }) => {
        ...
        project?.screenshots?.forEach((screenshot) =>
          images.push(new Request("/api/img?url=" + screenshot))
        );
        return images;
      }) ?? [];
    ...
    await cache
      .addAll(images.flat())
      .then(() => console.log("Cached project images"));
  } catch (e) {
...
const handleProjectImageFetchRequest = (e) => {
  e.respondWith(
    (async () => {
      try {
        // get cached response
        const cachedResult = await caches.match(
          "/api/img?url=" + e.request.url
        );
        if (cachedResult) {
          return cachedResult;
        }
...
const handleProjectFetchRequest = (e) => {
  e.respondWith(
    fetch(e.request)
      ...
      .catch((err) => {
        console.log(
          "Failed to fetch project data, trying to retrieve from localForage"
        );
        ...
        return localforage
          ...
          .catch((localForageErr) => {
            console.error(
              "Error retrieving data from localForage:",
              localForageErr
            );
            return new Response(
              JSON.stringify({
                error: "Failed to fetch data and retrieve local data",
              }),
              {
                headers: { "Content-Type": "application/json" },
                status: 500,
  ...
  e.respondWith(
    fetch(e.request)
      .then(async (response) => {
        ...
        // cache projects in localforage
        data.forEach((project) => {
          localforage.setItem(project.project.name, project);
        });
...
self.addEventListener("install", (event) => {
  event.waitUntil(
    (async () => {
      ...
      await cache
        .addAll(APPSHELL_FILES)
        .catch((e) => console.error("Failed to cache links", e)),
        cacheProjectImages(),
